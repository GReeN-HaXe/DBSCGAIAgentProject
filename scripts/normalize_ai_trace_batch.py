...
from src.agent.trace_summary import build_review_trace_payload, build_training_trace_rows
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Batch-normalize raw AI traces into review/training artifacts.")
    parser.add_argument(
        "--input-glob",
        type=str,
        default="artifacts/phase22_source/ai_match_trace_*.json",
        help="Glob pattern for raw AI trace JSON files.",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
    ...
    parser.add_argument(
        "--summary-output",
        type=Path,
        default=Path("artifacts/phase22_source/ai_trace_normalization_summary.json"),
    ...
    args.summary_output.write_text(
        json.dumps(
            {
                "schema_version": "ai_trace_normalization_batch.v1",
