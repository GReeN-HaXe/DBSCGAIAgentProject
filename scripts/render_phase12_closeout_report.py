...
def main() -> None:
    parser = argparse.ArgumentParser(description="Render a Phase 12 closeout markdown report.")
    parser.add_argument("--manifest", type=Path, required=True, help="Phase 12 manifest JSON path.")
    parser.add_argument("--history-summary", type=Path, default=None, help="Optional Phase 12 history summary JSON path.")
    parser.add_argument("--output", type=Path, default=Path("artifacts/phase12_closeout_report.md"), help="Markdown output path.")
    ...
    lines = ["# Phase 12 Closeout", ""]
    lines.append("## Visual Recognition")
    ...
    lines.append(f"- crop_dataset: `{artifacts.get('crop_dataset', '')}`")
