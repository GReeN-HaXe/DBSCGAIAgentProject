...
from src.agent import has_torchvision_support


def _run(cmd: list[str], *, stage_name: str, timeout_seconds: int) -> float:
    print(f"[phase17-backbone] start: {stage_name}")
    print(f"[phase17-backbone] cmd: {' '.join(cmd)}")
    ...
    elapsed = time.perf_counter() - started
    print(f"[phase17-backbone] done: {stage_name} ({elapsed:.1f}s)")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train and evaluate the Phase 17 pretrained-backbone pipeline.")
    parser.add_argument("--dataset", type=Path, default=Path("artifacts/phase13_reference_image_dataset.json"))
    parser.add_argument("--artifacts-dir", type=Path, default=Path("artifacts/phase17_backbone"))
    parser.add_argument("--run-name", type=str, default="phase17_backbone_run")
    ...
    model_path = artifacts_dir / "phase17_resnet18_model.json"
    retrieval_path = artifacts_dir / "phase17_resnet18_retrieval.json"
    manifest_path = artifacts_dir / "phase17_backbone_manifest.json"

    stage_timings = [
        {
            "stage": "train_resnet18_model",
            "duration_seconds": _run(
                [
                    sys.executable,
                    str((ROOT / "scripts" / "train_phase17_resnet18_model.py").resolve()),
                    "--dataset",
                    str(dataset),
                    "--split",
                    "train",
                ...
                stage_name="train_resnet18_model",
                timeout_seconds=int(args.stage_timeout_seconds),
            ),
        },
        {
            "stage": "evaluate_resnet18_retrieval",
            "duration_seconds": _run(
                [
                    sys.executable,
                ...
                stage_name="evaluate_resnet18_retrieval",
                timeout_seconds=int(args.stage_timeout_seconds),
            ),
        },
    ]
    ...
    if not isinstance(recall, dict):
        recall = {}
    manifest = {
        "schema_version": "phase17.backbone_manifest.v1",
        "status": "pass",
