...
def _candidate_card_ids(db_path: Path, *, limit: int | None) -> list[int]:
    ...
    try:
        sql = (
            ...
            "OR LOWER(COALESCE(card_skill_unstyled, '')) LIKE '%[awaken]%' "
            "OR LOWER(COALESCE(card_skill_unstyled, '')) LIKE '%[union-%' "
            "OR LOWER(COALESCE(card_skill_unstyled, '')) LIKE '%[swap%' "
            "OR LOWER(COALESCE(card_skill_unstyled, '')) LIKE '%[evolve%' "
            "OR LOWER(COALESCE(card_skill_unstyled, '')) LIKE '%[xeno-evolve%' "
            "OR LOWER(COALESCE(card_skill_unstyled, '')) LIKE '%[ex-evolve%' "
            "OR LOWER(COALESCE(card_skill_unstyled, '')) LIKE '%[revive%' "
            "OR LOWER(COALESCE(card_skill_unstyled, '')) LIKE '%[over realm%' "
            "OR LOWER(COALESCE(card_skill_unstyled, '')) LIKE '%[dark over realm%' "
            "OR LOWER(COALESCE(card_skill_unstyled, '')) LIKE '%[arrival%' "
            "OR LOWER(COALESCE(card_skill_unstyled, '')) LIKE '%[bond %' "
            ") "
            "ORDER BY id"
        )
