...
from src.agent import compare_phase20_vs_phase17_retrieval, has_torchvision_support


def _run(cmd: list[str], *, stage_name: str, timeout_seconds: int) -> float:
    print(f"[phase20-backbone] start: {stage_name}")
    print(f"[phase20-backbone] cmd: {' '.join(cmd)}")
    ...
    elapsed = time.perf_counter() - started
    print(f"[phase20-backbone] done: {stage_name} ({elapsed:.1f}s)")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train and evaluate the Phase 20 hybrid raw-image pipeline.")
    parser.add_argument("--dataset", type=Path, default=Path("artifacts/phase13_reference_image_dataset.json"))
    parser.add_argument("--artifacts-dir", type=Path, default=Path("artifacts/phase20_backbone"))
    parser.add_argument("--run-name", type=str, default="phase20_backbone_run")
    ...
    model_path = artifacts_dir / "phase20_resnet18_hybrid_model.json"
    retrieval_path = artifacts_dir / "phase20_resnet18_hybrid_retrieval.json"
    phase17_path = artifacts_dir / "phase17_resnet18_retrieval.json"
    compare_path = artifacts_dir / "phase20_vs_phase17_compare.json"
    manifest_path = artifacts_dir / "phase20_backbone_manifest.json"

    stage_timings = [
        {
            "stage": "train_phase20_resnet18_hybrid",
            "duration_seconds": _run(
                [
                    sys.executable,
                    str((ROOT / "scripts" / "train_phase20_resnet18_hybrid_model.py").resolve()),
                    "--dataset",
                    str(dataset),
                    "--split",
                    "train",
                    ...
                    str(float(args.classification_weight)),
                    "--triplet-weight",
                    str(float(args.triplet_weight)),
                    "--weights-mode",
                ...
                stage_name="train_phase20_resnet18_hybrid",
                timeout_seconds=int(args.stage_timeout_seconds),
            ),
        },
        {
            "stage": "evaluate_phase20_resnet18_hybrid_retrieval",
            "duration_seconds": _run(
                [
                    sys.executable,
                    str((ROOT / "scripts" / "evaluate_phase20_resnet18_hybrid_retrieval.py").resolve()),
                    "--model",
                    str(model_path),
                    "--dataset",
                    str(dataset),
                ...
                stage_name="evaluate_phase20_resnet18_hybrid_retrieval",
                timeout_seconds=int(args.stage_timeout_seconds),
            ),
        },
        {
    ...
    compare = compare_phase20_vs_phase17_retrieval(phase20_retrieval=retrieval, phase17_retrieval=phase17)
    ...
    if not isinstance(recall, dict):
        recall = {}
    manifest = {
        "schema_version": "phase20.backbone_manifest.v1",
        "status": "pass",
        ...
        "metrics": {
            ...
            "phase20_vs_phase17_mrr_lift": float(compare.get("mrr_lift", 0.0) or 0.0),
            "phase20_vs_phase17_recall_at_1_lift": float(compare.get("recall_at_1_lift", 0.0) or 0.0),
        },
        "training": {
