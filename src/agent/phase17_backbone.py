...
PHASE17_MODEL_SCHEMA_VERSION = "phase17.resnet18_classifier.v1"
PHASE17_RETRIEVAL_SCHEMA_VERSION = "phase17.resnet18_retrieval.v1"


def has_torchvision_support() -> bool:
...
def _require_torchvision() -> Any:
    if not has_torchvision_support():
        raise RuntimeError("PyTorch/torchvision are not installed. Install requirements-torch.txt, then rerun.")
    import torch
    import torch.nn as nn
    import torch.nn.functional as F
    from torchvision import models

    return {"torch": torch, "nn": nn, "F": F, "models": models}
...
def _apply_reference_view(image: Image.Image, reference_view: str) -> Image.Image:
...
def _prepare_examples(dataset: dict[str, Any], *, split: str, max_examples: int = 0) -> dict[str, Any]:
...
def _image_to_tensor(path: str, *, image_size: int, reference_view: str, normalize_mean: tuple[float, float, float], normalize_std: tuple[float, float, float]) -> list[float]:
    image = Image.open(path).convert("RGB")
    image = _apply_reference_view(image, reference_view)
    image = image.resize((int(image_size), int(image_size)))
    pixels = list(image.getdata())
    values: list[float] = []
    means = list(normalize_mean)
    stds = list(normalize_std)
    for r, g, b in pixels:
        channels = [r / 255.0, g / 255.0, b / 255.0]
        for idx, channel in enumerate(channels):
            values.append((channel - means[idx]) / stds[idx])
    return values
...
def _build_resnet18(torchvision_mods: Any, *, label_count: int, weights_mode: str) -> tuple[Any, bool]:
...
def _build_feature_extractor(torchvision_mods: Any, model_payload: dict[str, Any], *, device: str) -> Any:
    ...
    models = torchvision_mods["models"]
    mode = str(model_payload.get("weights_mode", "default"))
    if mode == "default":
...
def train_phase17_resnet18_model(
    ...
    *,
    ...
    model, weights_loaded = _build_resnet18(mods, label_count=len(label_vocab), weights_mode=str(weights_mode))
    ...
    for epoch in range(max(1, int(epochs))):
        ...
        if progress_every > 0 and (((epoch + 1) % progress_every) == 0 or (epoch + 1) == int(epochs)):
            print(f"[phase17-train] epoch {epoch + 1}/{int(epochs)} loss={average_loss:.6f}")

    return {
        "schema_version": PHASE17_MODEL_SCHEMA_VERSION,
...
def evaluate_phase17_resnet18_retrieval(
    ...
    *,
    ...
    return {
        "schema_version": PHASE17_RETRIEVAL_SCHEMA_VERSION,
