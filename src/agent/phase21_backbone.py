...
PHASE21_MODEL_SCHEMA_VERSION = "phase21.efficientnet_b0_classifier.v1"
PHASE21_RETRIEVAL_SCHEMA_VERSION = "phase21.efficientnet_b0_retrieval.v1"
PHASE21_COMPARE_SCHEMA_VERSION = "phase21.compare.v1"


def _build_efficientnet_b0(torchvision_mods: Any, *, label_count: int, weights_mode: str) -> tuple[Any, bool]:
    ...
    model.classifier[1] = nn.Linear(in_features, int(label_count))
...
def _build_feature_extractor(torchvision_mods: Any, model_payload: dict[str, Any], *, device: str) -> Any:
    ...
    base.classifier = nn.Sequential(nn.Dropout(p=0.2, inplace=True), nn.Identity())
    state_dict = _load_state_dict(torch, dict(model_payload.get("state_dict", {})))
    filtered_state = {key: value for key, value in state_dict.items() if not key.startswith("classifier.1.")}
...
def train_phase21_efficientnet_b0_model(
    ...
    *,
    ...
    model, weights_loaded = _build_efficientnet_b0(mods, label_count=len(label_vocab), weights_mode=str(weights_mode))
    ...
    for epoch in range(max(1, int(epochs))):
        ...
        if progress_every > 0 and (((epoch + 1) % progress_every) == 0 or (epoch + 1) == int(epochs)):
            print(f"[phase21-train] epoch {epoch + 1}/{int(epochs)} loss={average_loss:.6f}")

    return {
        "schema_version": PHASE21_MODEL_SCHEMA_VERSION,
...
def evaluate_phase21_efficientnet_b0_retrieval(
    ...
    *,
    ...
    return {
        "schema_version": PHASE21_RETRIEVAL_SCHEMA_VERSION,
...
def compare_phase21_vs_phase17_retrieval(*, phase21_retrieval: dict[str, Any], phase17_retrieval: dict[str, Any]) -> dict[str, Any]:
    phase21_count = int(phase21_retrieval.get("example_count", 0) or 0)
    phase17_count = int(phase17_retrieval.get("example_count", 0) or 0)
    if phase21_count != phase17_count:
        raise ValueError("example-count mismatch between phase21 and phase17 retrieval payloads")
    phase21_recall = phase21_retrieval.get("recall_at_k", {})
    phase17_recall = phase17_retrieval.get("recall_at_k", {})
    if not isinstance(phase21_recall, dict):
        phase21_recall = {}
    if not isinstance(phase17_recall, dict):
        phase17_recall = {}
    phase21_mrr = float(phase21_retrieval.get("mean_reciprocal_rank", 0.0) or 0.0)
    phase17_mrr = float(phase17_retrieval.get("mean_reciprocal_rank", 0.0) or 0.0)
    phase21_r1 = float(phase21_recall.get("1", 0.0) or 0.0)
    phase17_r1 = float(phase17_recall.get("1", 0.0) or 0.0)
    phase21_r5 = float(phase21_recall.get("5", 0.0) or 0.0)
    phase17_r5 = float(phase17_recall.get("5", 0.0) or 0.0)
    phase21_r10 = float(phase21_recall.get("10", 0.0) or 0.0)
    phase17_r10 = float(phase17_recall.get("10", 0.0) or 0.0)
    return {
        "schema_version": PHASE21_COMPARE_SCHEMA_VERSION,
        "target_type": "card_identity",
        "example_count": phase21_count,
        "phase21_model_name": str(phase21_retrieval.get("model_name", "")),
        "phase17_model_name": str(phase17_retrieval.get("model_name", "")),
        "phase21_mean_reciprocal_rank": phase21_mrr,
        "phase17_mean_reciprocal_rank": phase17_mrr,
        "phase21_recall_at_1": phase21_r1,
        "phase17_recall_at_1": phase17_r1,
        "phase21_recall_at_5": phase21_r5,
        "phase17_recall_at_5": phase17_r5,
        "phase21_recall_at_10": phase21_r10,
        "phase17_recall_at_10": phase17_r10,
        "mrr_lift": phase21_mrr - phase17_mrr,
        "recall_at_1_lift": phase21_r1 - phase17_r1,
        "recall_at_5_lift": phase21_r5 - phase17_r5,
        "recall_at_10_lift": phase21_r10 - phase17_r10,
        "phase21_wins": (
            phase21_mrr > phase17_mrr
            or phase21_r1 > phase17_r1
            or phase21_r5 > phase17_r5
            or phase21_r10 > phase17_r10
        ),
    }
