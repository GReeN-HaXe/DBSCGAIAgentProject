...
def test_phase19_resnet50_train_and_eval_or_runtime_error(tmp_path: Path) -> None:
    ...
    if not has_torchvision_support():
        with pytest.raises(RuntimeError, match="requirements-torch.txt"):
            train_phase19_resnet50_model(dataset, epochs=1, batch_size=2, image_size=32, max_examples=3, weights_mode="none")
        return

    model = train_phase19_resnet50_model(
        dataset,
        epochs=1,
        batch_size=2,
        image_size=32,
    ...
    assert model["schema_version"] == PHASE19_MODEL_SCHEMA_VERSION
    retrieval = evaluate_phase19_resnet50_retrieval(
        model,
        dataset,
        gallery_split="train",
    ...
    assert retrieval["schema_version"] == PHASE19_RETRIEVAL_SCHEMA_VERSION
    assert retrieval["example_count"] == 3


def test_phase19_compare_helper() -> None:
    payload = compare_phase19_vs_phase17_retrieval(
        phase19_retrieval={
            "example_count": 100,
    ...
    assert payload["schema_version"] == PHASE19_COMPARE_SCHEMA_VERSION
    assert payload["phase19_wins"] is True
