...
def test_phase20_hybrid_train_and_eval_or_runtime_error(tmp_path: Path) -> None:
    ...
    if not has_torchvision_support():
        with pytest.raises(RuntimeError, match="requirements-torch.txt"):
            train_phase20_resnet18_hybrid_model(dataset, epochs=1, steps_per_epoch=1, batch_size=2, image_size=32, max_examples=6, weights_mode="none")
        return

    model = train_phase20_resnet18_hybrid_model(
        dataset,
        epochs=1,
        steps_per_epoch=2,
        batch_size=2,
    ...
    assert model["schema_version"] == PHASE20_MODEL_SCHEMA_VERSION
    retrieval = evaluate_phase20_resnet18_hybrid_retrieval(
        model,
        dataset,
        gallery_split="train",
    ...
    assert retrieval["schema_version"] == PHASE20_RETRIEVAL_SCHEMA_VERSION
    assert retrieval["example_count"] == 3


def test_phase20_compare_helper() -> None:
    payload = compare_phase20_vs_phase17_retrieval(
        phase20_retrieval={
            "example_count": 100,
    ...
    assert payload["schema_version"] == PHASE20_COMPARE_SCHEMA_VERSION
    assert payload["phase20_wins"] is True
